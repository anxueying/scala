  ...
  /**
    ...
    */
  object PopularCategoriesDemo4 extends BaseApp{
    ...
    def main(args: Array[String]): Unit = {
      runApp{
        ...
        val joinResult = clickResult.fullOuterJoin(orderResult).fullOuterJoin(payResult)

        val result = joinResult.map({
          case (categoryId,(option,payCount)) =>
            (categoryId,option.map(_._1).flatten.getOrElse(0),option.map(_._2).flatten.getOrElse(0),payCount.getOrElse(0))
        })
